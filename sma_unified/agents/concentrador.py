...
class AgenteConcentrador:
    ...
    def crear_tarea(self, observaciones: List[Dict], proyecto_info: Dict, texto_proyecto_ley: str = "") -> Task:
        """Crear tarea CrewAI de concentración desde tasks.yaml"""
        from sma_unified.config import load_tasks_yaml
        tasks_cfg = load_tasks_yaml()
        task_info = tasks_cfg.get('tarea_concentrar_observaciones') or tasks_cfg.get('tareas', {}).get('tarea_concentrar_observaciones') or tasks_cfg.get('tareas', {}).get('Concentrar_Observaciones', {})
        
        desc_template = task_info.get('description') or task_info.get('descripcion')
        expected_output = task_info.get('expected_output', "Dictamen Consolidado de Observaciones estructurado")
        
        texto_ley = texto_proyecto_ley or proyecto_info.get('texto') or proyecto_info.get('contenido_texto') or proyecto_info.get('titulo', 'Sin texto')
        if desc_template and "{texto_proyecto_ley}" in desc_template:
            prompt = desc_template.replace("{texto_proyecto_ley}", texto_ley[:4000])
            prompt += f"\n\nOBSERVACIONES TÉCNICAS Y DICTÁMENES PREVIOS:\n{json.dumps(observaciones, indent=2, ensure_ascii=False)}"
        else:
            prompt = f"""
